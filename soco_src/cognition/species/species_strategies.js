    ...
    // --- 4. Ecologist (Diversity) ---
     ecologist: function(stand_data, agent, activityType) {
        ...
        if (activityType === 'planting') {
            ...
            // Filter ALL_VALID to find candidates that are NOT present
            var candidates = SpeciesData.ALL_VALID.filter(function(s) { 
                return present_ids.indexOf(s) === -1; 
            });

            // FIX: Plant a mix of up to 3 missing species
            var count_to_plant = 3;
            var picks = [];
            ...
            if (candidates.length > 0) {
                ...
                picks = SpeciesData.getMultipleRandom('ALL_VALID', count_to_plant);
            }
